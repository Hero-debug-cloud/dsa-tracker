...
export function AddAttemptModal({
  ...
  const [attemptProblemId, setAttemptProblemId] =
    useState<string>(defaultProblemId);
  ...
  const handleAddAttempt = async () => {
    if (!attemptUser || !attemptProblemId || !attemptDate) {
      toast.error(
        "Please fill in all required fields (User, Problem ID, Date)",
      );
      return;
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        ...
        <div className="grid gap-6 py-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={attemptFirstTry}
                onValueChange={setAttemptFirstTry}
              >
        ...
        <DialogFooter>
          ...
          <Button onClick={handleAddAttempt} disabled={isSubmitting}>
            {isSubmitting ? (
